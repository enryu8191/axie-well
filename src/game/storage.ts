...
export interface Preferences { best: number; muted: boolean }
export function readPreferences(): Preferences {
  try {
    const value = JSON.parse(localStorage.getItem(KEY) || '{}');
    return { best: Number.isSafeInteger(value.best) && value.best > 0 ? value.best : 0, muted: value.muted === true };
  } catch { return { best: 0, muted: false }; }
}
